...
def transform_data(df):
    # df = pd.read_json("/Users/admin/Desktop/Softeer_DE_6th/missions/W1/Countries_by_GDP.json")
...
def load_region(df, save_path):
    df.to_json(save_path + "Countries_with_region.json", orient="records", indent=4, force_ascii = False)

def filter_data(path):
    gdp_df = pd.read_json(path + "Countries_by_GDP.json")
    region_df = pd.read_json(path + "Countries_with_region.json")

    # GDP가 100B USD 이상이 되는 국가만 구해서 화면에 출력
    filtered_df = gdp_df[gdp_df["GDP_USD_billion"] > 100]
    print("GDP가 100B USD 이상이 되는 국가:")
    print(filtered_df[['Country', 'GDP_USD_billion']])

    # 각 Region별로 top5 국가의 평균을 구해서 화면에 출력

     # 두 데이터를 Country 기준으로 merge
    merged_df = pd.merge(gdp_df, region_df, on="Country")
    region_group = merged_df.groupby("Region")
    
    top_5_means = []
    for region, group in region_group:
        sorted_df = group.sort_values(by = "GDP_USD_billion", ascending = False)
        top_5 = sorted_df.head(5)
        mean_GDP = top_5["GDP_USD_billion"].mean()
        top_5_means.append((region, mean_GDP))

    top_5_means_df = pd.DataFrame(top_5_means, columns=['Region', 'GDP_means'])
    top_5_means_df['GDP_means'] = top_5_means_df['GDP_means'].round(2)
    top_5_means_df = top_5_means_df.sort_values(by = "GDP_means", ascending=False)

    print("각 Region별 top5 국가의 평균:")
    print(top_5_means_df)
...
def main():
    ...
    with open(path + "etl_project_log.txt", "a") as log_file:
        ...
        try:
            ...
            # 저장
            df.to_json(path + "Countries_by_GDP.json", orient="records", indent=4, force_ascii = False)
    ...
    try:
        region_df = extract_region()
        region_df = transform_region(region_df)
        load_region(region_df, path)
